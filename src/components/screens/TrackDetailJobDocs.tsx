...
const jobDocsSteps = [
  {
    ...
    description: 'Review dan tanda tangan kontrak dengan employer'
  },
  {
    id: '2',
    title: 'Sertifikat keahlian',
    status: 'completed' as const,
    description: 'Upload sertifikat caregiver dan bahasa Jepang N4'
  },
  {
    id: '3',
    title: 'Surat rekomendasi',
    status: 'active' as const,
    description: 'Surat rekomendasi dari training center'
  },
  {
    id: '4',
    title: 'Background check',
    status: 'locked' as const,
    description: 'Verifikasi latar belakang oleh employer'
  }
];

export default function TrackDetailJobDocs() {
  const [activeTab, setActiveTab] = useState('beranda');

  const completedSteps = jobDocsSteps.filter(s => s.status === 'completed').length;
  const totalSteps = jobDocsSteps.length;
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-page)] max-w-[375px] mx-auto relative pb-[106px]">
      ...
      <main className="px-5 py-4 space-y-4">
        <div>
          <h2 className="mb-1">Dokumen Kerja</h2>
          <p className="text-[14px] text-[var(--text-secondary)]">
            Dokumen khusus untuk pekerjaan sebagai Caregiver
          </p>
        </div>
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-3 px-4 py-4 ${
                    index < jobDocsSteps.length - 1 ? 'border-b border-[var(--border-subtle)]' : ''
                  }`}
                >
        ...
        <Card variant="default">
          <div className="text-center py-2">
            <p className="text-[14px] leading-[22px] text-[var(--text-secondary)] mb-3">
              Ada pertanyaan tentang dokumen kerja? 💬
            </p>
