...
const medicalSteps = [
  {
    id: '1',
    title: 'Medical check-up umum',
    status: 'completed' as const,
    description: 'Pemeriksaan kesehatan dasar di klinik partner'
  },
  {
    id: '2',
    title: 'Tes laboratorium',
    status: 'completed' as const,
    description: 'Tes darah, urin, rontgen dada'
  },
  {
    id: '3',
    title: 'Sertifikat kesehatan',
    status: 'completed' as const,
    description: 'Dokumen sertifikat dari dokter'
  },
  {
    id: '4',
    title: 'Verifikasi dokumen',
    status: 'active' as const,
    description: 'Admin Gapai verifikasi kelengkapan dokumen (2-3 hari)'
  }
];

export default function TrackDetailMedical() {
  const [activeTab, setActiveTab] = useState('beranda');

  const completedSteps = medicalSteps.filter(s => s.status === 'completed').length;
  const totalSteps = medicalSteps.length;
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-page)] max-w-[375px] mx-auto relative pb-[106px]">
      ...
      <main className="px-5 py-4 space-y-4">
        <div>
          <h2 className="mb-1">Medical Check-up</h2>
          <p className="text-[14px] text-[var(--text-secondary)]">
            Pemeriksaan kesehatan untuk keberangkatan
          </p>
        </div>
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-3 px-4 py-4 ${
                    index < medicalSteps.length - 1 ? 'border-b border-[var(--border-subtle)]' : ''
                  }`}
                >
        ...
        <Card variant="default">
          <div className="text-center py-2">
            <p className="text-[14px] leading-[22px] text-[var(--text-secondary)] mb-3">
              Ada pertanyaan tentang medical check-up? 💬
            </p>
